...
if __name__ == '__main__':
  try:
    ...
    order_log_file = order_file.split('.')[0] + '_log.yml'
    ...
    for key, val in orders.items():
      ...
      place_order_at = (datetime.utcnow() -
                        timedelta(seconds=1)).strftime('%Y%m%d-%H:%M:%S.%f')
      order_logs[key] = {'place_order_at': place_order_at}

      #log_file_handler.write('Test case: {}\n'.format(key))
      #log_file_handler.write('Place order at: {}\n'.format(place_order_at))
